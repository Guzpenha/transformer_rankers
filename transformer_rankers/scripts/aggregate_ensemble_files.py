...
def main():
    ...
    n_runs=5
    n_ns_and_dataset=5
    num_finished_runs = len(sorted_folders)//(n_runs* n_ns_and_dataset)
    ensemble_id = 0
    for dataset_i in range(0, num_finished_runs * (n_runs* n_ns_and_dataset), (n_runs* n_ns_and_dataset)):
        for setting_i in range(n_ns_and_dataset):
            preds_dfs = []
            preds_dfs_softmax = []
            os.makedirs(args.model_output_folder+"_agg/{}".format(ensemble_id), exist_ok=True)
            print(ensemble_id)
            for seed_i in range(n_runs):
                i=dataset_i+setting_i+(5 * seed_i)
                run_folder = sorted_folders[i]
                print(run_folder)
                ...
                with open(run_folder+"/config.json") as f:
                    ...
                    config["num_foward_prediction_passes"] = n_runs
            ...
            avg_df = sum_df/n_runs
            avg_df_softmax = sum_df_softmax/n_runs
