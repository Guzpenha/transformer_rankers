...
def main():
    ...
    # ns_test_tct_colbert = negative_sampling.DenseRetrievalNegativeSamplerPyserini(list(train["response"].values)+list(test["response"].values), args.num_ns,
    #             args.data_folder+args.task+"/anserini_test_{}/".format(args.sample_data), args.sample_data, args.anserini_folder)
    ...
    ns_info = [
        ...
        # (ns_test_tct_colbert, ["cand_tctColBERT_{}".format(i) for i in range(args.num_ns)] + ["tctColBERT_retrieved_relevant", "tctColBERT_rank"], 'tctColBERT'),
    ...
    print("R@10")
    print(examples_df[[c for c in examples_df.columns if 'retrieved_relevant' in c]].sum()/examples_df.shape[0])
    print("R@1")
    print(examples_df[examples_df["sentenceBERT_rank"]==0].shape[0]/examples_df.shape[0])
